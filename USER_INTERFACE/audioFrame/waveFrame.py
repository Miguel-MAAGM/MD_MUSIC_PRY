...
class waveFrame(ctk.CTkFrame):
    def __init__(self,master,clb_PLAY=None, fs=1000, T=1,**kwargs):
    ...
    def generar_onda(self, tipo):
        if tipo == "triangular":
            # Señal triangular
            triangular_freq = 3  # Frecuencia de la señal triangular (Hz)
            ...
            # Señal rectangular
            rectangular_freq = 4  # Frecuencia de la señal rectangular (Hz)
            ...
            # Señal senoidal
            sinusoidal_freq = 1  # Frecuencia de la señal senoidal (Hz)
    ...
    def getPoints(self,ofset,points):
        self.points=[]

        for n in range(points):
            self.points.append([self.t[self.idx+ofset*n],self.signal[self.idx+ofset*n]])
        if not self.idx>len(self.t):
            self.idx+=10
        else:
            self.idx=0
        return  self.points
    ...
    def visualizar_recta(self,array):
        colores = ['r', 'g', 'b', 'c', 'm', 'y', 'k']  # Colores disponibles
        
        time = [tupla[0] for tupla in array]
        signal = [tupla[1] for tupla in array]

        self.ax.scatter(time, signal, color='red', label='Intersecciones')
